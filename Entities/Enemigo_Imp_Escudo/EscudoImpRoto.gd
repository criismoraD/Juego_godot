extends Node3D
# Escudo roto del Imp con físicas y disolución

# === Parámetros asignados desde ImpShieldGirl.gd (o exportados) ===
@export var material_escudo: Material = preload("res://Entities/Enemigo_Imp_Escudo/ESCUDO_IMP_MAT.tres")
...
func _convertir_a_rigidbody(mesh_instance: MeshInstance3D):
	...
	# Collision shape: convex hull del mesh real
	...
	# Material: asignar por surface
	if mesh_instance.mesh:
		for surface_idx in range(mesh_instance.mesh.get_surface_count()):
			var mat = material_escudo.duplicate() if material_escudo else StandardMaterial3D.new()
	...
	# El eje Z (perpendicular a la cámara) es el giro principal.
	# Los ejes X e Y tienen un suave tambaleo para dar profundidad.
...
func _finish_dissolve():
	...
	# Eliminar los RigidBody3D de los trozos
	for rb in _rigid_bodies:
		if is_instance_valid(rb):
