...
@export var escena_escudo_roto: PackedScene = preload("res://Entities/Enemies/ImpShieldGirl/EscudoImpRoto.tscn")
...
func _buscar_enemigo_a_proteger():
	...
	for enemy in enemies:
		...
		# Proteger cualquier enemigo activo (que no esté muriendo o muerto)
...
func _process_walking(_delta):
	...
	# Verificar que el enemigo siga vivo y activo
	if (
		enemigo_protegido is EnemyBase
		and (
			enemigo_protegido.current_state == EnemyBase.State.DYING
			or enemigo_protegido.current_state == EnemyBase.State.DEAD
		)
	):
		# Enemigo murió, resetear para buscar otro en el siguiente frame
		enemigo_protegido = null
		return
...
func _process_defending(_delta):
	...
	# Buscar un nuevo enemigo si el actual ya no es válido o murió
	if not enemigo_protegido or not is_instance_valid(enemigo_protegido):
		_buscar_enemigo_a_proteger()
	else:
		if enemigo_protegido is EnemyBase:
			if (
				enemigo_protegido.current_state == EnemyBase.State.DYING
				or enemigo_protegido.current_state == EnemyBase.State.DEAD
			):
	...
	# Si hay un enemigo válido, evaluar si debemos movernos para mantener la protección
	if enemigo_protegido and is_instance_valid(enemigo_protegido):
		var target_x = enemigo_protegido.global_position.x - distancia_proteccion
		# Si el enemigo se movió a la izquierda y estamos a su derecha, volver a caminar para adelantarnos
		if global_position.x > target_x + 0.15:
			_cambiar_estado(State.WALKING)
...
func _play_animation(anim_name: String, custom_blend: float = -1.0, speed: float = 1.0):
	...
	for possible_anim in possible_names:
		if anim_player.has_animation(possible_anim):
			...
			if "IMP_ESCUDO_MUERTE02" in possible_anim:
...
func _get_animation_duration(anim_name: String) -> float:
	...
	for possible_anim in possible_names:
		if anim_player.has_animation(possible_anim):
			var duration = anim_player.get_animation(possible_anim).length
			if "IMP_ESCUDO_MUERTE02" in possible_anim:
				duration -= 0.5
			return duration
