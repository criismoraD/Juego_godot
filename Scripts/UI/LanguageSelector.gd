...
func _ready() -> void:
	...
	tween.tween_property(fade_overlay, "color:a", 0.0, 0.6).set_ease(Tween.EASE_OUT).set_trans(
		Tween.TRANS_SINE
	)
...
func _create_language_buttons() -> void:
	for lang_code in LANGUAGES:
		...
		btn.add_theme_stylebox_override(
			"normal", _make_style(Color(0.15, 0.1, 0.2, 0.9), Color(0.7, 0.55, 0.3, 0.8), 0)
		)
		btn.add_theme_stylebox_override(
			"hover", _make_style(Color(0.3, 0.2, 0.1, 1), Color(0.95, 0.8, 0.45, 1), 8)
		)
		btn.add_theme_stylebox_override(
			"pressed", _make_style(Color(0.45, 0.3, 0.15, 1), Color(1, 0.9, 0.55, 1), 0)
		)
...
func _go_to_intro() -> void:
	...
	for child in grid.get_children():
		if child is Button:
			child.disabled = true
	var tween = create_tween()
	tween.tween_property(fade_overlay, "color:a", 1.0, 0.5).set_ease(Tween.EASE_IN).set_trans(
		Tween.TRANS_SINE
	)
	tween.finished.connect(
		func(): get_tree().change_scene_to_file("res://Scenes/UI/IntroScene.tscn")
	)
