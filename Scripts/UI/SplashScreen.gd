...
func _play_splash_sequence() -> void:
	...
	(
		tween
		. tween_property(fade_overlay, "color:a", 0.0, FADE_IN_DURATION)
	...
	(
		tween
		. parallel()
		. tween_property(splash_image, "scale", Vector2(ZOOM_TO, ZOOM_TO), ZOOM_DURATION)
	...
	(
		tween
		. tween_property(fade_overlay, "color:a", 1.0, FADE_OUT_DURATION)
