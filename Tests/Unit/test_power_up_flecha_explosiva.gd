...
func test_power_up_initialization():
	...
	assert_eq(_power_up.municion_jugador, 10, "Debe otorgar 10 flechas al jugador")
	assert_eq(_power_up.municion_aliadas, 5, "Debe otorgar 5 flechas a cada aliada")
	assert_eq(_power_up.tiempo_auto_consumo, 3.0, "El tiempo de auto-consumo debe ser de 3.0 segundos")
	assert_eq(_power_up.velocidad_rotacion, 180.0, "La velocidad de rotación continua debe ser de 180 grados/segundo")
...
func test_probabilidades_drop_lonko_y_goblin():
	...
	assert_not_null(lonko.flecha_explosiva_powerup_scene, "Lonko debe tener asignada la escena del power-up")
