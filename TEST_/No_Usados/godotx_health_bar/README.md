<p align="center">
    <a href="https://github.com/godot-x/health-bar" target="_blank" rel="noopener noreferrer">
        <img width="300" src="extras/images/logo.png" alt="GodotX Health Bar - Logo">
    </a>
</p>
...
- **Dual usage**: GodotxHealthBarControl (UI) and GodotxHealthBar2D (world overlay)
...
- **Full styling**: GodotxHealthBarStyle resource (colors, roundness, fill inset, border, shadow, gradient)
...
1. Copy the `addons/godotx_health_bar` folder into your Godot project.
2. Enable the plugin in **Project → Project Settings → Plugins**.
3. Add **GodotxHealthBarControl** (UI) or **GodotxHealthBar2D** (world) from the Add Node dialog.

## Usage

### UI (HUD)

Add a **GodotxHealthBarControl** node under any Control (e.g. CanvasLayer or HUD). Use anchors/layout as usual. Set `value` (0–100) or use `min_value`/`max_value` and set value in that range; the bar normalizes to 0–100 for drawing.

```gdscript
var bar: GodotxHealthBarControl = $GodotxHealthBarControl
...
Add a **GodotxHealthBar2D** as child of your scene (or of a dedicated UI layer). Set `bar_size` (pixels), optionally set `follow_target` to a Node2D and `follow_offset` so the bar follows the character.

```gdscript
var bar: GodotxHealthBar2D = $GodotxHealthBar2D
...
Open **scenes/demo/godotx_health_bar_demo.tscn** and run the project (or set it as main scene and press F5). The demo includes:

- **HUD section**: GodotxHealthBarControl with value slider and controls for roundness, border, fill inset, animation, thresholds, label/icon position and offsets, shadow, gradient.
- **World section**: Simple character and GodotxHealthBar2D above it with follow target; value and character position sliders.
...
- **GodotxHealthBarControl** (Control): `style`, `value`, `min_value`, `max_value`; `set_value(v, animate)`, `get_value()`; value signals.
- **GodotxHealthBar2D** (Node2D): Same value API; `bar_size`, `follow_target`, `follow_offset`, `billboard_scale`.
- **GodotxHealthBarStyle** (Resource): All visual and animation options; save as `.tres` and reuse.
- **GodotxHealthBarEnums**: LabelPosition, IconPosition, GradientDirection, AnimationMode, IconFitMode.
