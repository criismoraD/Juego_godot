...
func _ready():
	# DEBUG TRANSICIONES: asegurar que el panel de transiciones se crea siempre
	call_deferred("_setup_debug_transiciones")
...
func _process(delta):
	...
	_actualizar_scroll_shape(delta)
...
func _crear_panel_controles_spawn() -> void:
	...
	btn_refuerzo_perrena.add_theme_font_size_override("font_size", 11)
	btn_refuerzo_perrena.pressed.connect(func(): if game_ui: game_ui._spawn_refuerzo_perrena_debug())
	grid_items.add_child(btn_refuerzo_perrena)
...
# ═══════════════════════════════════════════════════════════════════════════════
# DEBUG TRANSICIONES — Overlay + Panel de prueba del shader
# Copia de NIVEL01 con herramienta visual para probar shader/transition.gdshader
# ═══════════════════════════════════════════════════════════════════════════════
const RUTA_SHADER_TRANSITION: String = "res://shader/transition.gdshader"
const CAPA_TRANSITION_DEBUG: int = 190

var _transition_layer: CanvasLayer = null
var _transition_rect: ColorRect = null
var _transition_mat: ShaderMaterial = null
var _transition_tween: Tween = null
var _debug_transiciones_inicializado: bool = false
var _duracion_transicion: float = 1.2
var _factor_label: Label = null
var _factor_slider: HSlider = null
var _width_slider: HSlider = null
var _tiling_slider: HSlider = null
var _feathering_slider: HSlider = null
var _treshold_slider: HSlider = null
var _color_picker: ColorPickerButton = null
var _gradient_fixed_check: CheckButton = null
var _gradient_texturas: Array[Texture2D] = []
var _shape_texturas: Array[Texture2D] = []
var _gradient_idx: int = 0
var _shape_idx: int = 0
var _btn_grad_ciclo: Button = null
var _btn_shape_ciclo: Button = null
var _scroll_animado: bool = false
var _scroll_time: float = 0.0

const NOMBRES_GRADIENTES: Array[String] = [
	"Izq→Der", "Der→Izq", "Arr→Abj", "Abj→Arr",
	"Diag ↘", "Diag ↖", "Radial ◉", "Radial ◎",
	"Cuadrado ▢", "Cuadrado ▣",
]
const NOMBRES_SHAPES: Array[String] = ["Perlin", "Cellular", "Simplex", "Liso"]
const RADIO_COBERTURA: float = 0.72

## Catálogo de tipos de transición (equivalente a los 12 pre-made del pack):
## cada tipo combina gradient + shape + parámetros para un look distinto.
const TIPOS_TRANSICION: Array[Dictionary] = [
	{"nombre": "➡ Wipe →", "grad": 0, "fixed": false, "shape": 3, "tiling": 32.0, "feather": 0.03, "tresh": 1.0, "width": 0.22, "color": Color.BLACK, "scroll": false},
	{"nombre": "⬅ Wipe ←", "grad": 1, "fixed": false, "shape": 3, "tiling": 32.0, "feather": 0.03, "tresh": 1.0, "width": 0.22, "color": Color.BLACK, "scroll": false},
	{"nombre": "⬇ Wipe ↓", "grad": 2, "fixed": false, "shape": 3, "tiling": 32.0, "feather": 0.03, "tresh": 1.0, "width": 0.22, "color": Color.BLACK, "scroll": false},
	{"nombre": "↘ Diagonal", "grad": 4, "fixed": false, "shape": 3, "tiling": 32.0, "feather": 0.05, "tresh": 1.0, "width": 0.28, "color": Color.BLACK, "scroll": false},
	{"nombre": "◉ Iris Out", "grad": 6, "fixed": true, "shape": 3, "tiling": 32.0, "feather": 0.02, "tresh": 1.0, "width": 0.3, "color": Color.BLACK, "scroll": false},
	{"nombre": "◎ Iris In", "grad": 7, "fixed": true, "shape": 3, "tiling": 32.0, "feather": 0.02, "tresh": 1.0, "width": 0.3, "color": Color.BLACK, "scroll": false},
	{"nombre": "▢ Cuadrado", "grad": 8, "fixed": true, "shape": 3, "tiling": 32.0, "feather": 0.02, "tresh": 1.0, "width": 0.3, "color": Color(0.05, 0.08, 0.22), "scroll": false},
	{"nombre": "▣ Cuadrado Inv", "grad": 9, "fixed": true, "shape": 3, "tiling": 32.0, "feather": 0.02, "tresh": 1.0, "width": 0.3, "color": Color(0.05, 0.08, 0.22), "scroll": false},
	{"nombre": "✦ Disol. Suave", "grad": 0, "fixed": false, "shape": 2, "tiling": 8.0, "feather": 0.45, "tresh": 1.0, "width": 0.55, "color": Color.BLACK, "scroll": false},
	{"nombre": "▦ Mosaico Voronoi", "grad": 2, "fixed": false, "shape": 1, "tiling": 20.0, "feather": 0.3, "tresh": 0.9, "width": 0.5, "color": Color(0.08, 0.22, 0.12), "scroll": false},
	{"nombre": "🌫 Niebla Mágica", "grad": 6, "fixed": true, "shape": 0, "tiling": 6.0, "feather": 0.6, "tresh": 1.0, "width": 0.7, "color": Color(0.28, 0.12, 0.35), "scroll": false},
	{"nombre": "🔥 Onda Fuego", "grad": 2, "fixed": false, "shape": 0, "tiling": 10.0, "feather": 0.35, "tresh": 1.0, "width": 0.55, "color": Color(0.75, 0.2, 0.05), "scroll": true},
]


func _setup_debug_transiciones() -> void:
	if _debug_transiciones_inicializado:
		return
	_debug_transiciones_inicializado = true
	_crear_texturas_debug_transicion()
	_crear_overlay_transicion()
	_crear_panel_debug_transiciones()
	_actualizar_resolucion_transicion()
	if not get_viewport().size_changed.is_connected(_actualizar_resolucion_transicion):
		get_viewport().size_changed.connect(_actualizar_resolucion_transicion)


func _crear_texturas_debug_transicion() -> void:
	# 10 gradientes = 10 tipos de barrido base (como el pack del autor):
	# la DIRECCIÓN/FORMA de la transición depende únicamente de esta textura.
	_gradient_texturas = [
		_crear_gradiente_lineal(Vector2(0.0, 0.5), Vector2(1.0, 0.5)),   # 0 Izq→Der
		_crear_gradiente_lineal(Vector2(1.0, 0.5), Vector2(0.0, 0.5)),   # 1 Der→Izq
		_crear_gradiente_lineal(Vector2(0.5, 0.0), Vector2(0.5, 1.0)),   # 2 Arr→Abj
		_crear_gradiente_lineal(Vector2(0.5, 1.0), Vector2(0.5, 0.0)),   # 3 Abj→Arr
		_crear_gradiente_lineal(Vector2(0.0, 0.0), Vector2(1.0, 1.0)),   # 4 Diag ↘
		_crear_gradiente_lineal(Vector2(1.0, 1.0), Vector2(0.0, 0.0)),   # 5 Diag ↖
		_crear_gradiente_radial_cuadrado(GradientTexture2D.FILL_RADIAL, false),   # 6 Radial Out
		_crear_gradiente_radial_cuadrado(GradientTexture2D.FILL_RADIAL, true),    # 7 Radial In
		_crear_gradiente_radial_cuadrado(GradientTexture2D.FILL_SQUARE, false),   # 8 Cuadrado Out
		_crear_gradiente_radial_cuadrado(GradientTexture2D.FILL_SQUARE, true),    # 9 Cuadrado In
	]

	var noise_perlin := FastNoiseLite.new()
	noise_perlin.noise_type = FastNoiseLite.TYPE_PERLIN
	noise_perlin.frequency = 0.02
	var noise_cell := FastNoiseLite.new()
	noise_cell.noise_type = FastNoiseLite.TYPE_CELLULAR
	noise_cell.frequency = 0.015
	var noise_simplex := FastNoiseLite.new()
	noise_simplex.noise_type = FastNoiseLite.TYPE_SIMPLEX_SMOOTH
	noise_simplex.frequency = 0.025

	_shape_texturas = [
		_crear_shape_ruido(noise_perlin),   # 0 Perlin
		_crear_shape_ruido(noise_cell),     # 1 Cellular (mosaico)
		_crear_shape_ruido(noise_simplex),  # 2 Simplex suave
		_crear_shape_liso(),                # 3 Liso (sin patrón)
	]


func _crear_gradiente_lineal(desde: Vector2, hasta: Vector2) -> GradientTexture2D:
	var g := Gradient.new()  # Por defecto: 0=negro, 1=blanco
...
func _crear_gradiente_radial_cuadrado(fill_tipo: int, invertido: bool) -> GradientTexture2D:
	var g := Gradient.new()  # 0=negro (centro), 1=blanco (borde)
	if invertido:
		g.set_color(0, Color.WHITE)
		g.set_color(1, Color.BLACK)
	# RADIO_COBERTURA > distancia a esquina (~0.57 en aspect fijo) para cubrir todo
	var t := GradientTexture2D.new()
	t.gradient = g
	t.width = 512
	t.height = 512
	...
	t.fill_to = Vector2(0.5 + RADIO_COBERTURA, 0.5)
	return t


func _crear_shape_ruido(ruido: FastNoiseLite) -> NoiseTexture2D:
...
func _crear_shape_liso() -> ImageTexture:
...
func _crear_overlay_transicion() -> void:
	if not ResourceLoader.exists(RUTA_SHADER_TRANSITION):
		push_warning("[DebugTransiciones] No se encontró shader: " + RUTA_SHADER_TRANSITION)
		return
	var shader: Shader = load(RUTA_SHADER_TRANSITION) as Shader
	if shader == null:
		push_warning("[DebugTransiciones] No se pudo cargar el shader de transición.")
		return

	_transition_layer = CanvasLayer.new()
	_transition_layer.name = "TransitionDebugLayer"
	_transition_layer.layer = CAPA_TRANSITION_DEBUG
	_transition_layer.process_mode = Node.PROCESS_MODE_ALWAYS
	add_child(_transition_layer)

	_transition_rect = ColorRect.new()
	_transition_rect.name = "TransitionOverlay"
	...
	_transition_mat.shader = shader
	# Valores por defecto del shader — punto de partida neutro
	_transition_mat.set_shader_parameter("base_color", Color.BLACK)
	_transition_mat.set_shader_parameter("node_resolution", Vector2(1920, 1080))
	_transition_mat.set_shader_parameter("factor", 1.0) # 1 = transparente (oculta)
	_transition_mat.set_shader_parameter("width", 0.4)
	_transition_mat.set_shader_parameter("gradient_texture", _gradient_texturas[0] if not _gradient_texturas.is_empty() else null)
	_transition_mat.set_shader_parameter("gradient_fixed", false)
	_transition_mat.set_shader_parameter("shape_texture", _shape_texturas[0] if not _shape_texturas.is_empty() else null)
	_transition_mat.set_shader_parameter("shape_tiling", 32.0)
	_transition_mat.set_shader_parameter("shape_rotation", 0.0)
	_transition_mat.set_shader_parameter("shape_scroll", Vector2.ZERO)
	_transition_mat.set_shader_parameter("shape_feathering", 0.0)
	_transition_mat.set_shader_parameter("shape_treshold", 1.0)
	_transition_rect.material = _transition_mat


func _actualizar_resolucion_transicion() -> void:
	if _transition_mat == null:
		return
	var vp_size: Vector2 = get_viewport().get_visible_rect().size
	if vp_size.x < 1 or vp_size.y < 1:
		vp_size = Vector2(1920, 1080)
	_transition_mat.set_shader_parameter("node_resolution", vp_size)


func _crear_panel_debug_transiciones() -> void:
	if _transition_layer == null:
		return

	# Botón flotante para mostrar/ocultar el panel (esquina superior izquierda)
	var btn_toggle := Button.new()
	btn_toggle.name = "BtnToggleTransicionPanel"
	btn_toggle.text = "🎬 TRANSICIONES"
	btn_toggle.custom_minimum_size = Vector2(150, 32)
	btn_toggle.anchor_left = 0.0
	btn_toggle.anchor_top = 0.0
	btn_toggle.anchor_right = 0.0
	btn_toggle.anchor_bottom = 0.0
	btn_toggle.offset_left = 12
	btn_toggle.offset_top = 12
	btn_toggle.offset_right = 162
	btn_toggle.offset_bottom = 44
	var toggle_style := StyleBoxFlat.new()
	toggle_style.bg_color = Color(0.18, 0.16, 0.28, 0.97)
	toggle_style.set_corner_radius_all(6)
	toggle_style.set_border_width_all(2)
	toggle_style.border_color = Color(0.65, 0.55, 0.9, 0.9)
	btn_toggle.add_theme_stylebox_override("normal", toggle_style)
	btn_toggle.focus_mode = Control.FOCUS_NONE
	_transition_layer.add_child(btn_toggle)

	var panel := PanelContainer.new()
	panel.name = "DebugTransicionPanel"
	...
	btn_toggle.pressed.connect(func():
		panel.visible = not panel.visible
		btn_toggle.text = "✖ CERRAR" if panel.visible else "🎬 TRANSICIONES"
	)
	var panel_style := StyleBoxFlat.new()
	...
	panel_style.set_content_margin_all(10)
	panel_style.set_border_width_all(2)
	panel_style.border_color = Color(0.45, 0.42, 0.65, 0.9)
	panel.add_theme_stylebox_override("panel", panel_style)
	_transition_layer.add_child(panel)
	...
	lbl_titulo.text = "🎬 DEBUG TRANSICIONES"
	lbl_titulo.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	lbl_titulo.add_theme_color_override("font_color", Color(0.75, 0.65, 1.0))
	lbl_titulo.add_theme_font_size_override("font_size", 14)
	vbox_main.add_child(lbl_titulo)
	var lbl_sub := Label.new()
	lbl_sub.text = "shader/transition.gdshader"
	lbl_sub.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	lbl_sub.add_theme_color_override("font_color", Color(0.7, 0.7, 0.75))
	lbl_sub.add_theme_font_size_override("font_size", 10)
	vbox_main.add_child(lbl_sub)
	vbox_main.add_child(HSeparator.new())

	# ——— Botones principales de reproducción ———
	var lbl_play := Label.new()
	lbl_play.text = "▶ REPRODUCIR"
	lbl_play.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	lbl_play.add_theme_color_override("font_color", Color(1.0, 0.85, 0.3))
	lbl_play.add_theme_font_size_override("font_size", 11)
	vbox_main.add_child(lbl_play)

	var hbox_play := HBoxContainer.new()
	hbox_play.add_theme_constant_override("separation", 4)
	vbox_main.add_child(hbox_play)

	var btn_in := Button.new()
	btn_in.text = "⬛ CUBRIR (IN)"
	btn_in.custom_minimum_size = Vector2(0, 28)
	btn_in.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	btn_in.add_theme_font_size_override("font_size", 11)
	btn_in.focus_mode = Control.FOCUS_NONE
	var s_in := StyleBoxFlat.new()
	s_in.bg_color = Color(0.15, 0.15, 0.18, 1.0)
	s_in.set_corner_radius_all(4)
	s_in.set_border_width_all(1)
	s_in.border_color = Color(0.9, 0.9, 0.9, 0.3)
	btn_in.add_theme_stylebox_override("normal", s_in)
	btn_in.pressed.connect(func(): _play_transicion(true))
	hbox_play.add_child(btn_in)

	var btn_out := Button.new()
	btn_out.text = "🔲 REVELAR (OUT)"
	btn_out.custom_minimum_size = Vector2(0, 28)
	btn_out.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	btn_out.add_theme_font_size_override("font_size", 11)
	btn_out.focus_mode = Control.FOCUS_NONE
	var s_out := StyleBoxFlat.new()
	s_out.bg_color = Color(0.15, 0.35, 0.18, 1.0)
	s_out.set_corner_radius_all(4)
	btn_out.add_theme_stylebox_override("normal", s_out)
	btn_out.pressed.connect(func(): _play_transicion(false))
	hbox_play.add_child(btn_out)

	var hbox_play2 := HBoxContainer.new()
	hbox_play2.add_theme_constant_override("separation", 4)
	vbox_main.add_child(hbox_play2)

	var btn_loop := Button.new()
	btn_loop.text = "🔄 LOOP IN→OUT"
	btn_loop.custom_minimum_size = Vector2(0, 26)
	btn_loop.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	btn_loop.add_theme_font_size_override("font_size", 11)
	btn_loop.focus_mode = Control.FOCUS_NONE
	btn_loop.pressed.connect(func(): _play_transicion_loop())
	hbox_play2.add_child(btn_loop)

	var btn_toggle_vis := Button.new()
	btn_toggle_vis.text = "👁 TOGGLE"
	btn_toggle_vis.custom_minimum_size = Vector2(0, 26)
	btn_toggle_vis.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	btn_toggle_vis.add_theme_font_size_override("font_size", 11)
	btn_toggle_vis.focus_mode = Control.FOCUS_NONE
	btn_toggle_vis.pressed.connect(func(): _toggle_transicion_instant())
	hbox_play2.add_child(btn_toggle_vis)

	vbox_main.add_child(HSeparator.new())

	# ——— Presets de color / estilo ———
	var lbl_presets := Label.new()
	lbl_presets.text = "🎨 PRESETS (color + estilo)"
	lbl_presets.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	lbl_presets.add_theme_color_override("font_color", Color(0.7, 0.85, 1.0))
	lbl_presets.add_theme_font_size_override("font_size", 11)
	vbox_main.add_child(lbl_presets)

	var grid_presets := GridContainer.new()
	grid_presets.columns = 3
	grid_presets.add_theme_constant_override("h_separation", 4)
	grid_presets.add_theme_constant_override("v_separation", 4)
	vbox_main.add_child(grid_presets)

	var presets: Array[Dictionary] = [
		{"nombre": "⬛ Negro", "color": Color.BLACK, "width": 0.4, "tiling": 32.0, "feather": 0.0, "treshold": 1.0},
		{"nombre": "⬜ Blanco", "color": Color.WHITE, "width": 0.35, "tiling": 24.0, "feather": 0.15, "treshold": 0.9},
		{"nombre": "🟥 Rojo", "color": Color(0.8, 0.15, 0.18, 1.0), "width": 0.5, "tiling": 16.0, "feather": 0.2, "treshold": 1.0},
		{"nombre": "🟩 Bosque", "color": Color(0.08, 0.22, 0.12, 1.0), "width": 0.45, "tiling": 28.0, "feather": 0.1, "treshold": 1.0},
		{"nombre": "🟦 Noche", "color": Color(0.05, 0.08, 0.22, 1.0), "width": 0.6, "tiling": 40.0, "feather": 0.05, "treshold": 1.0},
		{"nombre": "🟪 Niebla", "color": Color(0.28, 0.12, 0.35, 1.0), "width": 0.7, "tiling": 12.0, "feather": 0.35, "treshold": 0.85},
	]
	for p in presets:
		var preset_copia: Dictionary = p.duplicate()
		var b := Button.new()
		b.text = preset_copia["nombre"]
		b.custom_minimum_size = Vector2(100, 24)
		b.add_theme_font_size_override("font_size", 11)
		b.focus_mode = Control.FOCUS_NONE
		var sb := StyleBoxFlat.new()
		sb.bg_color = (preset_copia["color"] as Color).lerp(Color(0.2, 0.2, 0.22, 1.0), 0.55)
		sb.set_corner_radius_all(4)
		sb.set_border_width_all(1)
		sb.border_color = preset_copia["color"]
		b.add_theme_stylebox_override("normal", sb)
		b.pressed.connect(_ejecutar_preset_demo.bind(preset_copia))
		grid_presets.add_child(b)

	# ——— TIPOS DE TRANSICIÓN (12 presets como el pack del autor) ———
	vbox_main.add_child(HSeparator.new())
	var lbl_tipos := Label.new()
	lbl_tipos.text = "🎞 TIPOS DE TRANSICIÓN"
	lbl_tipos.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	lbl_tipos.add_theme_color_override("font_color", Color(0.55, 1.0, 0.75))
	lbl_tipos.add_theme_font_size_override("font_size", 11)
	vbox_main.add_child(lbl_tipos)

	var grid_tipos_t := GridContainer.new()
	grid_tipos_t.columns = 3
	grid_tipos_t.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	grid_tipos_t.add_theme_constant_override("h_separation", 4)
	grid_tipos_t.add_theme_constant_override("v_separation", 4)
	vbox_main.add_child(grid_tipos_t)
	for i in range(TIPOS_TRANSICION.size()):
		var tipo: Dictionary = TIPOS_TRANSICION[i]
		var b := Button.new()
		b.text = tipo["nombre"]
		b.tooltip_text = "Grad: %s • Shape: %s" % [
			NOMBRES_GRADIENTES[int(tipo["grad"])], NOMBRES_SHAPES[int(tipo["shape"])]
		]
		b.custom_minimum_size = Vector2(100, 24)
		b.add_theme_font_size_override("font_size", 10)
		b.focus_mode = Control.FOCUS_NONE
		var st := StyleBoxFlat.new()
		st.bg_color = Color(0.14, 0.13, 0.2, 1.0)
		st.set_corner_radius_all(4)
		st.set_border_width_all(1)
		st.border_color = Color(0.5, 0.45, 0.75, 0.8)
		b.add_theme_stylebox_override("normal", st)
		b.pressed.connect(_ejecutar_tipo_transicion.bind(tipo))
		grid_tipos_t.add_child(b)

	# Fila de texturas gradient / shape
	var hbox_tex := HBoxContainer.new()
	hbox_tex.add_theme_constant_override("separation", 4)
	vbox_main.add_child(hbox_tex)
	var btn_grad := Button.new()
	btn_grad.text = "↔ Gradient: " + NOMBRES_GRADIENTES[0]
	btn_grad.custom_minimum_size = Vector2(0, 22)
	btn_grad.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	btn_grad.add_theme_font_size_override("font_size", 10)
	btn_grad.focus_mode = Control.FOCUS_NONE
	_btn_grad_ciclo = btn_grad
	btn_grad.pressed.connect(func():
		_gradient_idx = (_gradient_idx + 1) % _gradient_texturas.size()
		if is_instance_valid(_btn_grad_ciclo):
			_btn_grad_ciclo.text = "↔ Gradient: " + NOMBRES_GRADIENTES[_gradient_idx]
		if _transition_mat:
			_transition_mat.set_shader_parameter("gradient_texture", _gradient_texturas[_gradient_idx])
	)
	hbox_tex.add_child(btn_grad)
	var btn_shape := Button.new()
	btn_shape.text = "▦ Shape: " + NOMBRES_SHAPES[0]
	btn_shape.custom_minimum_size = Vector2(0, 22)
	btn_shape.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	btn_shape.add_theme_font_size_override("font_size", 10)
	btn_shape.focus_mode = Control.FOCUS_NONE
	_btn_shape_ciclo = btn_shape
	btn_shape.pressed.connect(func():
		_shape_idx = (_shape_idx + 1) % _shape_texturas.size()
		if is_instance_valid(_btn_shape_ciclo):
			_btn_shape_ciclo.text = "▦ Shape: " + NOMBRES_SHAPES[_shape_idx]
		if _transition_mat:
			_transition_mat.set_shader_parameter("shape_texture", _shape_texturas[_shape_idx])
	)
	hbox_tex.add_child(btn_shape)

	vbox_main.add_child(HSeparator.new())

	# ——— Controles deslizantes ———
	var lbl_ctrl := Label.new()
	lbl_ctrl.text = "⚙ PARÁMETROS DEL SHADER"
	lbl_ctrl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	lbl_ctrl.add_theme_color_override("font_color", Color(0.7, 0.85, 1.0))
	lbl_ctrl.add_theme_font_size_override("font_size", 11)
	vbox_main.add_child(lbl_ctrl)

	# Factor (0 = cubierto, 1 = transparente)
	var hbox_factor := HBoxContainer.new()
	hbox_factor.add_theme_constant_override("separation", 6)
	vbox_main.add_child(hbox_factor)
	var lbl_factor := Label.new()
	lbl_factor.text = "Factor"
	lbl_factor.custom_minimum_size = Vector2(62, 0)
	lbl_factor.add_theme_font_size_override("font_size", 10)
	hbox_factor.add_child(lbl_factor)
	_factor_slider = HSlider.new()
	_factor_slider.min_value = 0.0
	_factor_slider.max_value = 1.0
	_factor_slider.step = 0.01
	_factor_slider.value = 1.0
	_factor_slider.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_factor_slider.focus_mode = Control.FOCUS_NONE
	hbox_factor.add_child(_factor_slider)
	_factor_label = Label.new()
	_factor_label.text = "1.00"
	_factor_label.custom_minimum_size = Vector2(32, 0)
	_factor_label.add_theme_font_size_override("font_size", 10)
	hbox_factor.add_child(_factor_label)
	_factor_slider.value_changed.connect(func(v: float):
		if _transition_mat:
			_transition_mat.set_shader_parameter("factor", v)
		if is_instance_valid(_factor_label):
			_factor_label.text = "%.2f" % v
	)

	# Width
	var hbox_w := HBoxContainer.new()
	hbox_w.add_theme_constant_override("separation", 6)
	vbox_main.add_child(hbox_w)
	var lbl_w := Label.new()
	lbl_w.text = "Width"
	lbl_w.custom_minimum_size = Vector2(62, 0)
	lbl_w.add_theme_font_size_override("font_size", 10)
	hbox_w.add_child(lbl_w)
	_width_slider = HSlider.new()
	_width_slider.min_value = 0.05
	_width_slider.max_value = 1.0
	_width_slider.step = 0.01
	_width_slider.value = 0.4
	_width_slider.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_width_slider.focus_mode = Control.FOCUS_NONE
	hbox_w.add_child(_width_slider)
	var lbl_w_val := Label.new()
	lbl_w_val.text = "0.40"
	lbl_w_val.custom_minimum_size = Vector2(32, 0)
	lbl_w_val.add_theme_font_size_override("font_size", 10)
	hbox_w.add_child(lbl_w_val)
	_width_slider.value_changed.connect(func(v: float):
		if _transition_mat:
			_transition_mat.set_shader_parameter("width", v)
		lbl_w_val.text = "%.2f" % v
	)

	# Shape tiling
	var hbox_tiling := HBoxContainer.new()
	hbox_tiling.add_theme_constant_override("separation", 6)
	vbox_main.add_child(hbox_tiling)
	var lbl_t := Label.new()
	lbl_t.text = "Tiling"
	lbl_t.custom_minimum_size = Vector2(62, 0)
	lbl_t.add_theme_font_size_override("font_size", 10)
	hbox_tiling.add_child(lbl_t)
	_tiling_slider = HSlider.new()
	_tiling_slider.min_value = 1.0
	_tiling_slider.max_value = 64.0
	_tiling_slider.step = 1.0
	_tiling_slider.value = 32.0
	_tiling_slider.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_tiling_slider.focus_mode = Control.FOCUS_NONE
	hbox_tiling.add_child(_tiling_slider)
	var lbl_t_val := Label.new()
	lbl_t_val.text = "32"
	lbl_t_val.custom_minimum_size = Vector2(32, 0)
	lbl_t_val.add_theme_font_size_override("font_size", 10)
	hbox_tiling.add_child(lbl_t_val)
	_tiling_slider.value_changed.connect(func(v: float):
		if _transition_mat:
			_transition_mat.set_shader_parameter("shape_tiling", v)
		lbl_t_val.text = "%d" % int(v)
	)

	# Shape feathering
	var hbox_feather := HBoxContainer.new()
	hbox_feather.add_theme_constant_override("separation", 6)
	vbox_main.add_child(hbox_feather)
	var lbl_f := Label.new()
	lbl_f.text = "Feather"
	lbl_f.custom_minimum_size = Vector2(62, 0)
	lbl_f.add_theme_font_size_override("font_size", 10)
	hbox_feather.add_child(lbl_f)
	_feathering_slider = HSlider.new()
	_feathering_slider.min_value = 0.0
	_feathering_slider.max_value = 1.0
	_feathering_slider.step = 0.01
	_feathering_slider.value = 0.0
	_feathering_slider.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_feathering_slider.focus_mode = Control.FOCUS_NONE
	hbox_feather.add_child(_feathering_slider)
	var lbl_f_val := Label.new()
	lbl_f_val.text = "0.00"
	lbl_f_val.custom_minimum_size = Vector2(32, 0)
	lbl_f_val.add_theme_font_size_override("font_size", 10)
	hbox_feather.add_child(lbl_f_val)
	_feathering_slider.value_changed.connect(func(v: float):
		if _transition_mat:
			_transition_mat.set_shader_parameter("shape_feathering", v)
		lbl_f_val.text = "%.2f" % v
	)

	# Shape treshold
	var hbox_tresh := HBoxContainer.new()
	hbox_tresh.add_theme_constant_override("separation", 6)
	vbox_main.add_child(hbox_tresh)
	var lbl_th := Label.new()
	lbl_th.text = "Treshold"
	lbl_th.custom_minimum_size = Vector2(62, 0)
	lbl_th.add_theme_font_size_override("font_size", 10)
	hbox_tresh.add_child(lbl_th)
	_treshold_slider = HSlider.new()
	_treshold_slider.min_value = 0.0
	_treshold_slider.max_value = 2.0
	_treshold_slider.step = 0.01
	_treshold_slider.value = 1.0
	_treshold_slider.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_treshold_slider.focus_mode = Control.FOCUS_NONE
	hbox_tresh.add_child(_treshold_slider)
	var lbl_th_val := Label.new()
	lbl_th_val.text = "1.00"
	lbl_th_val.custom_minimum_size = Vector2(32, 0)
	lbl_th_val.add_theme_font_size_override("font_size", 10)
	hbox_tresh.add_child(lbl_th_val)
	_treshold_slider.value_changed.connect(func(v: float):
		if _transition_mat:
			_transition_mat.set_shader_parameter("shape_treshold", v)
		lbl_th_val.text = "%.2f" % v
	)

	# Duración
	var hbox_dur := HBoxContainer.new()
	hbox_dur.add_theme_constant_override("separation", 6)
	vbox_main.add_child(hbox_dur)
	var lbl_dur := Label.new()
	lbl_dur.text = "Duración"
	lbl_dur.custom_minimum_size = Vector2(62, 0)
	lbl_dur.add_theme_font_size_override("font_size", 10)
	hbox_dur.add_child(lbl_dur)
	var dur_slider := HSlider.new()
	dur_slider.min_value = 0.2
	dur_slider.max_value = 4.0
	dur_slider.step = 0.1
	dur_slider.value = _duracion_transicion
	dur_slider.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	dur_slider.focus_mode = Control.FOCUS_NONE
	hbox_dur.add_child(dur_slider)
	var lbl_dur_val := Label.new()
	lbl_dur_val.text = "%.1fs" % _duracion_transicion
	lbl_dur_val.custom_minimum_size = Vector2(32, 0)
	lbl_dur_val.add_theme_font_size_override("font_size", 10)
	hbox_dur.add_child(lbl_dur_val)
	dur_slider.value_changed.connect(func(v: float):
		_duracion_transicion = v
		lbl_dur_val.text = "%.1fs" % v
	)

	vbox_main.add_child(HSeparator.new())

	# Color + gradient_fixed
	var hbox_color := HBoxContainer.new()
	hbox_color.alignment = BoxContainer.ALIGNMENT_CENTER
	hbox_color.add_theme_constant_override("separation", 6)
	vbox_main.add_child(hbox_color)
	var lbl_col := Label.new()
	lbl_col.text = "Base color:"
	lbl_col.add_theme_font_size_override("font_size", 10)
	hbox_color.add_child(lbl_col)
	_color_picker = ColorPickerButton.new()
	_color_picker.color = Color.BLACK
	_color_picker.custom_minimum_size = Vector2(80, 24)
	_color_picker.focus_mode = Control.FOCUS_NONE
	hbox_color.add_child(_color_picker)
	_color_picker.color_changed.connect(func(c: Color):
		if _transition_mat:
			_transition_mat.set_shader_parameter("base_color", c)
	)
	_gradient_fixed_check = CheckButton.new()
	_gradient_fixed_check.text = "Gradient fixed"
	_gradient_fixed_check.button_pressed = false
	_gradient_fixed_check.focus_mode = Control.FOCUS_NONE
	_gradient_fixed_check.add_theme_font_size_override("font_size", 10)
	hbox_color.add_child(_gradient_fixed_check)
	_gradient_fixed_check.toggled.connect(func(pressed: bool):
		if _transition_mat:
			_transition_mat.set_shader_parameter("gradient_fixed", pressed)
	)

	vbox_main.add_child(HSeparator.new())

	# Botones extra de utilidad
	var hbox_util := HBoxContainer.new()
	hbox_util.add_theme_constant_override("separation", 4)
	vbox_main.add_child(hbox_util)
	var btn_reset := Button.new()
	btn_reset.text = "↺ Reset params"
	btn_reset.custom_minimum_size = Vector2(0, 24)
	...
	btn_reset.pressed.connect(func(): _reset_params_transicion())
	hbox_util.add_child(btn_reset)
	var btn_hide := Button.new()
	btn_hide.text = "👁 Ocultar overlay"
	btn_hide.custom_minimum_size = Vector2(0, 24)
	btn_hide.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	btn_hide.add_theme_font_size_override("font_size", 10)
	btn_hide.focus_mode = Control.FOCUS_NONE
	btn_hide.pressed.connect(func():
		if _transition_mat:
			_transition_mat.set_shader_parameter("factor", 1.0)
			if is_instance_valid(_factor_slider):
				_factor_slider.value = 1.0
	)
	hbox_util.add_child(btn_hide)

	var lbl_hint := Label.new()
	lbl_hint.text = "💡 10 gradientes = 10 direcciones/formas.\nUsa 🎞 TIPOS para ver los presets completos."
	lbl_hint.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	lbl_hint.add_theme_color_override("font_color", Color(0.6, 0.6, 0.65))
	lbl_hint.add_theme_font_size_override("font_size", 9)
	vbox_main.add_child(lbl_hint)


func _aplicar_preset_transicion(color: Color, width: float, tiling: float, feather: float, treshold: float) -> void:
	if _transition_mat == null:
		return
	_transition_mat.set_shader_parameter("base_color", color)
	_transition_mat.set_shader_parameter("width", width)
	_transition_mat.set_shader_parameter("shape_tiling", tiling)
	_transition_mat.set_shader_parameter("shape_feathering", feather)
	_transition_mat.set_shader_parameter("shape_treshold", treshold)
	if is_instance_valid(_color_picker):
		_color_picker.color = color
	if is_instance_valid(_width_slider):
		_width_slider.value = width
	if is_instance_valid(_tiling_slider):
		_tiling_slider.value = tiling
	if is_instance_valid(_feathering_slider):
		_feathering_slider.value = feather
	if is_instance_valid(_treshold_slider):
		_treshold_slider.value = treshold


func _ejecutar_preset_demo(preset: Dictionary) -> void:
	_aplicar_preset_transicion(preset["color"], preset["width"], preset["tiling"], preset["feather"], preset["treshold"])
	_play_transicion(true)
	get_tree().create_timer(_duracion_transicion + 0.15).timeout.connect(func():
		if is_instance_valid(self) and _transition_mat != null:
			_play_transicion(false)
	)


## Aplica un tipo de transición completo (gradient + shape + parámetros) y lo
## reproduce IN→OUT, replicando los presets del pack de Binbun.
func _ejecutar_tipo_transicion(tipo: Dictionary) -> void:
	if _transition_mat == null:
		return
	_gradient_idx = int(tipo.get("grad", 0))
	_shape_idx = int(tipo.get("shape", 3))
	if not _gradient_texturas.is_empty():
		_transition_mat.set_shader_parameter("gradient_texture", _gradient_texturas[_gradient_idx])
	if not _shape_texturas.is_empty():
		_transition_mat.set_shader_parameter("shape_texture", _shape_texturas[_shape_idx])
	var fijo: bool = bool(tipo.get("fixed", false))
	_transition_mat.set_shader_parameter("gradient_fixed", fijo)
	if is_instance_valid(_gradient_fixed_check):
		_gradient_fixed_check.set_pressed_no_signal(fijo)
	_scroll_animado = bool(tipo.get("scroll", false))
	if not _scroll_animado:
		_scroll_time = 0.0
		_transition_mat.set_shader_parameter("shape_scroll", Vector2.ZERO)
	_aplicar_preset_transicion(
		tipo.get("color", Color.BLACK),
		float(tipo.get("width", 0.4)),
		float(tipo.get("tiling", 32.0)),
		float(tipo.get("feather", 0.0)),
		float(tipo.get("tresh", 1.0))
	)
	_refrescar_labels_texturas()
	_play_transicion(true)
	get_tree().create_timer(_duracion_transicion + 0.15).timeout.connect(func():
		if is_instance_valid(self) and _transition_mat != null:
			_play_transicion(false)
...
func _refrescar_labels_texturas() -> void:
	if is_instance_valid(_btn_grad_ciclo):
		_btn_grad_ciclo.text = "↔ Gradient: " + NOMBRES_GRADIENTES[_gradient_idx]
	if is_instance_valid(_btn_shape_ciclo):
		_btn_shape_ciclo.text = "▦ Shape: " + NOMBRES_SHAPES[_shape_idx]
...
## Scroll animado del patrón shape (feature "movement" del pack del autor).
func _actualizar_scroll_shape(delta: float) -> void:
	if not _scroll_animado or _transition_mat == null:
		return
	_scroll_time += delta
	_transition_mat.set_shader_parameter(
		"shape_scroll", Vector2(_scroll_time * 0.08, _scroll_time * 0.03)
	)


func _reset_params_transicion() -> void:
	_aplicar_preset_transicion(Color.BLACK, 0.4, 32.0, 0.0, 1.0)
	if _transition_mat:
		_transition_mat.set_shader_parameter("factor", 1.0)
		_transition_mat.set_shader_parameter("gradient_fixed", false)
		_transition_mat.set_shader_parameter("shape_rotation", 0.0)
		_transition_mat.set_shader_parameter("shape_scroll", Vector2.ZERO)
		if not _gradient_texturas.is_empty():
			_transition_mat.set_shader_parameter("gradient_texture", _gradient_texturas[0])
		if not _shape_texturas.is_empty():
			_transition_mat.set_shader_parameter("shape_texture", _shape_texturas[0])
	_gradient_idx = 0
	_shape_idx = 0
	_scroll_animado = false
	_scroll_time = 0.0
	if is_instance_valid(_factor_slider):
		_factor_slider.value = 1.0
	if is_instance_valid(_gradient_fixed_check):
...
func _play_transicion(hacia_cubrir: bool, duracion: float = -1.0) -> void:
	if _transition_mat == null:
		return
	var dur: float = duracion if duracion > 0.0 else _duracion_transicion
	var target: float = 0.0 if hacia_cubrir else 1.0
	var current: float = float(_transition_mat.get_shader_parameter("factor"))
	# Evitar tween solapado
	if _transition_tween and _transition_tween.is_valid():
		_transition_tween.kill()
	_transition_tween = create_tween()
	_transition_tween.set_trans(Tween.TRANS_SINE)
	_transition_tween.set_ease(Tween.EASE_IN_OUT)
	_transition_tween.tween_method(_aplicar_factor_transicion, current, target, dur)


func _aplicar_factor_transicion(valor: float) -> void:
	if _transition_mat == null:
		return
	_transition_mat.set_shader_parameter("factor", valor)
	if is_instance_valid(_factor_slider) and not _factor_slider.has_focus():
		_factor_slider.value = valor
	if is_instance_valid(_factor_label):
		_factor_label.text = "%.2f" % valor


func _play_transicion_loop() -> void:
	_play_transicion(true)
	await get_tree().create_timer(_duracion_transicion + 0.1).timeout
	if not is_instance_valid(self) or _transition_mat == null:
		return
	_play_transicion(false)


func _toggle_transicion_instant() -> void:
	if _transition_mat == null:
		return
	var cur: float = float(_transition_mat.get_shader_parameter("factor"))
	var target: float = 1.0 if cur < 0.5 else 0.0
	_transition_mat.set_shader_parameter("factor", target)
	if is_instance_valid(_factor_slider):
		_factor_slider.value = target
	if is_instance_valid(_factor_label):
		_factor_label.text = "%.2f" % target
