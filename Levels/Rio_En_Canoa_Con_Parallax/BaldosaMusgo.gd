@tool
class_name BaldosaMusgo
extends Node3D

## Wrapper posicionable para la baldosa de musgo en el nivel del rio.
...
@export var material_baldosa: StandardMaterial3D:
	set(nuevo_material):
		material_baldosa = nuevo_material
		if is_node_ready():
			_aplicar_material()
...
func _aplicar_a_instancias(nodo: Node) -> void:
	if nodo is MeshInstance3D:
		...
		malla_instancia.material_override = material_baldosa
		if malla_instancia.mesh != null and malla_instancia.mesh.get_surface_count() > INDICE_SUPERFICIE_PRINCIPAL:
			malla_instancia.set_surface_override_material(INDICE_SUPERFICIE_PRINCIPAL, material_baldosa)
