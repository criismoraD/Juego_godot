@tool
class_name CasaBoneta
extends Node3D

## Wrapper posicionable para la casa boneta en el nivel del rio.
## Aplica el material con textura a todas las mallas importadas del GLB,
## sin depender del nombre interno del nodo importado.
## El desplazamiento parallax lo gestiona ParallaxFondoRio
## (misma velocidad que la cordillera).

const INDICE_SUPERFICIE_PRINCIPAL: int = 0

@export var material_casa: StandardMaterial3D:
	set(nuevo_material):
		material_casa = nuevo_material
		if is_node_ready():
			_aplicar_material()
...
func _aplicar_a_instancias(nodo: Node) -> void:
	if nodo is MeshInstance3D:
		...
		malla_instancia.material_override = material_casa
		if malla_instancia.mesh != null and malla_instancia.mesh.get_surface_count() > INDICE_SUPERFICIE_PRINCIPAL:
			malla_instancia.set_surface_override_material(INDICE_SUPERFICIE_PRINCIPAL, material_casa)
