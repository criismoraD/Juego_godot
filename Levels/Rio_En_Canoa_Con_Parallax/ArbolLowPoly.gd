@tool
class_name ArbolLowPoly
extends Node3D

## Wrapper posicionable para el árbol low poly en el nivel del rio.
...
@export var material_arbol: StandardMaterial3D:
	set(nuevo_material):
		material_arbol = nuevo_material
		if is_node_ready():
			_aplicar_material()
...
func _aplicar_a_instancias(nodo: Node) -> void:
	if nodo is MeshInstance3D:
		...
		malla_instancia.material_override = material_arbol
		if malla_instancia.mesh != null and malla_instancia.mesh.get_surface_count() > INDICE_SUPERFICIE_PRINCIPAL:
			malla_instancia.set_surface_override_material(INDICE_SUPERFICIE_PRINCIPAL, material_arbol)
