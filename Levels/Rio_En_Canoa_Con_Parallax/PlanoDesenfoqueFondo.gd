@tool
class_name PlanoDesenfoqueFondo
extends MeshInstance3D

## Capa 2D plana 3D que difumina todo lo que se encuentra detrás de ella al ser vista por la cámara.
## Puede posicionarse libremente en la escena (ejes X, Y, Z) para determinar con precisión qué elementos
## quedan nítidos en primer plano y cuáles quedan difuminados en la lejanía.
...
## Retorna el material de shader de desenfoque activo.
...
func _inicializar_material() -> void:
	if material_override is ShaderMaterial and (material_override as ShaderMaterial).shader == SHADER_DESENFOQUE:
		_material_desenfoque = material_override as ShaderMaterial
	else:
