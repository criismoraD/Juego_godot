@tool
class_name Naufragio
extends Node3D

## Wrapper posicionable para el naufragio en el nivel del rio.
...
@export var material_naufragio: StandardMaterial3D:
	set(nuevo_material):
		material_naufragio = nuevo_material
		if is_node_ready():
			_aplicar_material()
...
func _aplicar_a_instancias(nodo: Node) -> void:
	if nodo is MeshInstance3D:
		...
		malla_instancia.material_override = material_naufragio
		if malla_instancia.mesh != null and malla_instancia.mesh.get_surface_count() > INDICE_SUPERFICIE_PRINCIPAL:
			malla_instancia.set_surface_override_material(INDICE_SUPERFICIE_PRINCIPAL, material_naufragio)
