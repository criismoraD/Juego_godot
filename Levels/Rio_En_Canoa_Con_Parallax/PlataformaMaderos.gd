@tool
class_name PlataformaMaderos
extends Node3D

## Wrapper posicionable para la plataforma de tablones en el nivel del rio.
## Aplica el material con textura a todas las mallas importadas del GLB,
## sin depender del nombre interno del nodo importado.
## Incluye suelo físico (SueloPlataforma) del tamaño del modelo.

const INDICE_SUPERFICIE_PRINCIPAL: int = 0

@export var material_maderos: StandardMaterial3D:
	set(nuevo_material):
		material_maderos = nuevo_material
		if is_node_ready():
			_aplicar_material()
...
func _aplicar_a_instancias(nodo: Node) -> void:
	if nodo is MeshInstance3D:
		...
		malla_instancia.material_override = material_maderos
		if malla_instancia.mesh != null and malla_instancia.mesh.get_surface_count() > INDICE_SUPERFICIE_PRINCIPAL:
			malla_instancia.set_surface_override_material(INDICE_SUPERFICIE_PRINCIPAL, material_maderos)
