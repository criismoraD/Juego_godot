...
func _ready():
	...
	# Activar UI de Debug por defecto
	if game_ui:
	...
	# Iniciar oleadas libres (infinitas) directamente
...
func _on_nivel1_completado(_numero_oleada: int):
	...
	_log_debug("[NIVEL_DEBUG] ¡Oleada 4 completada! Mostrando victoria con botón continuar...")
...
func _actualizar_estado_spawner_label(estado: String):
	var panel = game_ui.get_node_or_null("DebugSpawnPanel")
	if not panel:
		return
